...
export function EnterpriseDashboard() {
  const { currentUser, accessibleDocuments, accessSummary, allDocuments } =
    useABAC();
        ...
        <div className="bg-white rounded-2xl border border-gray-100">
          ...
          <div className="p-5 space-y-4">
            {Object.entries(accessSummary.byDepartment).map(([dept, count]) => {
              const total = allDocuments.filter(
                d => d.attributes.department === dept
              ).length;
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-300 leading-relaxed">
              Your access to documents is determined by your department (
              {DEPARTMENT_LABELS[currentUser.attributes.department]}), clearance
              level ({currentUser.attributes.clearance}), and region (
              {currentUser.attributes.region}).
              {currentUser.attributes.managedDepartments &&
                currentUser.attributes.managedDepartments.length > 0 && (
                  <>
                    {' '}
                    You also have manager access to:{' '}
                    {currentUser.attributes.managedDepartments
